...
(() => {
  ...
  // Standalone deployment fallback. The omelette design runtime persists
  // dropped images to the sidecar via window.omelette.writeFile; outside it
  // (a plain static host) that bridge is absent, so we persist user uploads
  // to localStorage instead. The sidecar still loads as read-only defaults
  // (e.g. the hero photo), and localStorage overrides layer on top.
  const LS_KEY = 'dfs.image-slots';
  let _lsOk = null; // memoized — _render() (and thus this) fires on every resize tick
  function lsAvailable() {
    if (_lsOk !== null) return _lsOk;
    try {
      const k = '__dfs_ls_test__';
  ...
  function writeLocal() {
    try { window.localStorage.setItem(LS_KEY, JSON.stringify(slots)); } catch (e) {}
  }
  ...
  function load() {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .then((j) => {
        ...
        // localStorage (the user's own standalone uploads) layers between the
        // sidecar defaults and in-memory changes.
        const ls = readLocal();
        if ((j && typeof j === 'object') || ls) {
          const base = Object.assign({}, j && typeof j === 'object' ? j : null, ls);
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // Standalone (no omelette bridge) we persist to localStorage instead, so
      // uploads stay editable wherever localStorage is writable.
      const editable = !!(window.omelette && window.omelette.writeFile) || lsAvailable();
